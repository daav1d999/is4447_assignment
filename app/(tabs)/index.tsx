...
export default function IndexScreen() {
  ...
  const filteredHabits = habits.filter((habit: Habit) => {
    ...
    const matchesSearch = normalizedQuery.length === 0 || habit.name.toLowerCase().includes(normalizedQuery) || categoryName.toLowerCase().includes(normalizedQuery);
    ...
    if (selectedDateRange !== 'All') {
      ...
      matchesDateRange = habitLogs.some((log: HabitLog) => log.habitId === habit.id && log.logDate >= startDateString);
    }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <Text variant="bodyMedium" style={styles.subtitle}>Track, filter, and manage your habits</Text>
      ...
      <ScrollView contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}>
        {habits.length === 0 ? (
          <View style={styles.emptyState}>
            <Text variant="titleMedium" style={styles.emptyTitle}>No habits yet</Text>
            <Text variant="bodyMedium" style={styles.emptyBody}>Tap "Add Habit" to start tracking your first habit.</Text>
          </View>
        ) : filteredHabits.length === 0 ? (
          <View style={styles.emptyState}>
            <Text variant="titleMedium" style={styles.emptyTitle}>No results</Text>
            <Text variant="bodyMedium" style={styles.emptyBody}>Try changing your search or filters.</Text>
          </View>
        ) : (
          filteredHabits.map((habit: Habit) => (
            ...
            >
              <Card.Content>
                ...
                <Text variant="bodySmall" style={styles.meta}>{getCategoryName(habit.categoryId)}</Text>
                <Text variant="bodySmall" style={styles.meta}>{habit.targetType} target: {habit.targetValue}</Text>
              </Card.Content>
            </Card>
          ))
        )}
...
const styles = StyleSheet.create({
  safeArea: { backgroundColor: '#F8FAFC', flex: 1, paddingHorizontal: 18 },
  subtitle: { color: '#64748B', marginBottom: 10 },
  searchbar: { marginBottom: 10 },
  filterRow: { paddingBottom: 10, paddingRight: 8 },
  chip: { marginRight: 8 },
